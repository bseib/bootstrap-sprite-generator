...
class SpriteGenerator(object):
    ...
    sprite_less_item = """
        #.${prefix}-${name} { background-position: ${ox}spx ${oy}spx; }
        #
        """
    ...
    def __init__(self, options, filter, filter_type):
        ...
        self._prefix = options.prefix if options.prefix else 'icon'
    ...
    def _generate_sprite_less(self, icons, max_width, max_height):
        ...
        header = Template(self._get_as_text(self.sprite_less_header))
        sprite_file.write(header.substitute(dict(prefix=self._prefix, max_width=max_width, max_height=max_height)))
        
        # write each icon
        icon_line = Template(self._get_as_text(self.sprite_less_item))
        for (icon_name, icon) in icons.items():
            (ox, oy) = icon['location']
            sprite_file.write(icon_line.substitute(dict(prefix=self._prefix, name=icon_name, ox=-ox, oy=-oy)))
...
def main():
    ...
    parser.add_argument('-p', dest='prefix', help="place a string prefix before icon class names. The default is 'icon', e.g. the file asterisk.png will become CSS class '.icon-asterisk'. Example:  -p fugue ")
    ...
    parser.add_argument('-t', dest='type', help="sprite generator type. 'gi' is for glyphicons, 'gn' is for generic", choices=supported_types, required=True)
